"""Crawl herb names and links from 中医百科「中药大全」 page."""
...
TARGET_URL = f"{BASE_URL}/wiki/%E4%B8%AD%E8%8D%AF%E5%A4%A7%E5%85%A8"
OUTPUT_FILE = os.path.join(os.path.dirname(__file__), "中药大全.xlsx")
...
def parse_herb_links(html: str) -> list[tuple[str, str]]:
...
def main() -> int:
    try:
        rows = crawl()
        if not rows:
            print("未抓取到任何药材链接。", file=sys.stderr)
            return 1
